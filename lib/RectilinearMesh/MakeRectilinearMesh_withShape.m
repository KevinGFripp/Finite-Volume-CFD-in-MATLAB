function Mesh = MakeRectilinearMesh_withShape(Nx,Ny,Width,Height,growthx,growthy,Shapes)
Mesh.Nx = Nx;
Mesh.Ny = Ny;
Mesh.W = Width;
Mesh.H = Height;
...
Intervalsy = zeros(2*NUMSHAPES+2,1);
 ...
 for S = 1 : NUMSHAPES
   ...
   Intervalsx(S+1,1) = Shapes(S).u_grid_i(1);
   Intervalsx(S+2,1) = Shapes(S).u_grid_i(end);
 
   Intervalsy(S+1,1) = Shapes(S).v_grid_j(1);
   Intervalsy(S+2,1) = Shapes(S).v_grid_j(end);
