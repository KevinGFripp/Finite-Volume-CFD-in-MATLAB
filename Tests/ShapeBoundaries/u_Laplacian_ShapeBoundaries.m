function u_Laplacian_ShapeBoundaries()
%% include lib
basefolder = ...
...
Square = DefineBody(0.5,0.5,Width/Nx,Height/Ny,Nx,Ny,Width/Nx,Height/Ny);
...
[Lu,~] = uVelocityLaplacian_withShape(Mesh,Boundaries,Square);

 Datax = zeros((Nx+1)*Ny,1);

for i=1:Nx+1
    for j =1:Ny
    Datax(index(i,j,Nx+1)) = i + j;  
    end
end

figure(2)
imagesc(reshape(Lu*Datax,Nx+1,Ny))
view(-90,90)
